...
if [ ! -f "system/buildysb.prop" ]; then
  echo "! Failed: Extracted file system/buildysb.prop not found!"
  exit 1
fi
...
XVERSION=$(grep_prop version system/buildysb.prop)
XARCH=$(grep_prop arch system/buildysb.prop)
XMINSDK=$(grep_prop minsdk system/buildysb.prop)
XMAXSDK=$(grep_prop maxsdk system/buildysb.prop)
...
echo "- Placing files"
install_nobackup /system/buildysb.prop                      0    0 0644
install_nobackup /system/framework/okhttp.ya.jar       0    0 0644
...
install_nobackup  /system/lib/libya.so            0    0 0644
if [ $IS64BIT ]; then
  ...
  install_nobackup  /system/lib64/libya.so        0    0 0644
fi
